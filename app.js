...
recognition.onresult = function (event) {
  ...
  if(localStorage.getItem("lang") === "en-US"){
    ...
    if (transcript.includes("friday what's the current time")) {
      readOut(currentTime);
    }
    ...
    // jarvis bio
    if (transcript.toLowerCase().includes("tell about yourself")) { 
        readOut(
            "Sir, I am a Jarvis, a voice assistant made for browsers using JavaScript by Mr Shubh. I can do anything which can be done from a browser."
        );
    }
  ...
  // spotify
    ...
    if (transcript.includes("open firebase")) {
        readOut("opening firebase chaups");
  ...
  if(localStorage.getItem("lang") === "hi-IN"){
    ...
    if(transcript.includes("इंग्लिश में बदल")){
